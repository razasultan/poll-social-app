...
void main() {
  ...
  group('ShareablePollResultCard', () {
    testWidgets('renders the question, branding, options, and vote total', (tester) async {
      await tester.pumpWidget(_wrap(const ShareablePollResultCard(
    ...
    testWidgets('omits optional rows when post text, author, and share URL are absent', (tester) async {
      await tester.pumpWidget(_wrap(const ShareablePollResultCard(
        question: 'Coffee or tea?',
        entries: entries,
        totalVotes: 4,
    ...
    testWidgets('renders post text, author handle, and share URL when provided', (tester) async {
      await tester.pumpWidget(_wrap(const ShareablePollResultCard(
        question: 'Coffee or tea?',
        entries: entries,
        totalVotes: 4,
    ...
    testWidgets('uses the singular "vote" label for exactly one vote', (tester) async {
      await tester.pumpWidget(_wrap(const ShareablePollResultCard(
        question: 'Coffee or tea?',
        entries: [PollChartEntry(label: 'Coffee', count: 1, percentage: 100, selected: true)],
        totalVotes: 1,
      )));
