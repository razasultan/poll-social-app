...
void main() {
  group('PollOptionButton (pre-vote choice)', () {
    testWidgets('renders its label and reports taps', (tester) async {
      var tapped = false;
      await tester.pumpWidget(_wrap(PollOptionButton(
        label: 'Coffee',
    ...
    testWidgets('omits a media thumbnail when the option has no media', (tester) async {
      await tester.pumpWidget(_wrap(PollOptionButton(label: 'Plain', onPressed: () {})));
      expect(find.byType(OptionMediaThumbnail), findsNothing);
    });
  });

  group('PollResultBar (post-vote / expired results)', () {
    testWidgets('shows the option label, vote share, and progress bar', (tester) async {
      await tester.pumpWidget(_wrap(PollResultBar(
        label: 'Coffee',
        optionKey: 'opt-1',
        count: 3,
        totalVotes: 4,
  ...
  group('EngagementRow', () {
    testWidgets('renders engagement counts and reacts to like taps', (tester) async {
      var likeTapped = false;
      await tester.pumpWidget(_wrap(EngagementRow(
        votesCount: 12,
        likesCount: 5,
        commentsCount: 2,
        sharesCount: 1,
  ...
  group('PollMediaPreview (backward compatibility for media-less polls)', () {
    testWidgets('renders nothing when the poll has no main media', (tester) async {
  ...
  group('OptionMediaThumbnail', () {
    testWidgets('shows a play glyph over video options without fetching the network', (tester) async {
      await tester.pumpWidget(_wrap(const OptionMediaThumbnail(mediaUrl: null, mediaType: 'video')));
      expect(find.byIcon(Icons.play_arrow_rounded), findsOneWidget);
    });

    testWidgets('omits the play glyph for image options', (tester) async {
      await tester.pumpWidget(_wrap(const OptionMediaThumbnail(mediaUrl: null, mediaType: 'image')));
